...
Object.defineProperty(exports, "__esModule", { value: true });
const truncate_utf8_bytes_1 = __importDefault(__webpack_require__(/*! truncate-utf8-bytes */ "./node_modules/truncate-utf8-bytes/index.js"));
/**
...
const sanitize_filename_ts_1 = __webpack_require__(/*! sanitize-filename-ts */ "./node_modules/sanitize-filename-ts/build/index.js");
const transform_sensor_1 = __webpack_require__(/*! ./transform-sensor */ "./src/transform-sensor.ts");
...
function logWithMessage(msg) {
    log(msg);
    vscode.window.showInformationMessage(msg);
}
function logError(msg, errObject) {
    log(msg);
...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('hoganslendertanium.compareContentSet', () => __awaiter(this, void 0, void 0, function* () {
        ...
        const contentSet = yield vscode.window.showInputBox({
        ...
        const fqdn = yield vscode.window.showInputBox({
        ...
        const contentFilename = sanitize_filename_ts_1.sanitize(path.basename(parsed.pathname));
        ...
        (() => __awaiter(this, void 0, void 0, function* () {
            yield pipeline(got.stream(contentSet), fs.createWriteStream(contentSetFile));
            ...
            fs.readFile(contentSetFile, 'utf8', function (err, data) {
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (parser.validate(data) === true) { //optional (it'll return an object in case it's not valid)
                        ...
                        const serverDir = path.join(folderPath, `2 - ${sanitize_filename_ts_1.sanitize(fqdn)}`);
                        ...
                        jsonObj.content.sensor.forEach((sensor) => {
                            sensorHash.push(`${sensor.what_hash}`);
                            const name = sanitize_filename_ts_1.sanitize(sensor.name);
                            try {
                                const tmpSensor = transform_sensor_1.TransformSensor.transformContentSet(sensor);
                        ...
                        // get session
                        const { body } = yield got.post(`${restBase}/session/login`, {
                            json: {
                                username: username,
                                password: password,
                            },
                        ...
                        sensorHash.forEach((hash) => __awaiter(this, void 0, void 0, function* () {
                            const { body } = yield got.get(`${restBase}/sensors/by-hash/${hash}`, {
                                headers: {
                                    session: session,
                                },
                                responseType: 'json'
                            });
                            let sensor = body.data;
                            const name = sanitize_filename_ts_1.sanitize(sensor.name);
                            try {
                                sensor = transform_sensor_1.TransformSensor.transform(sensor);
                                const content = JSON.stringify(sensor, null, 2);
...
/***/ "./src/transform-metadata-item.ts":
/*!****************************************!*\
  !*** ./src/transform-metadata-item.ts ***!
  \****************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.TransformMetadataItem = void 0;
class TransformMetadataItem {
...
exports.TransformMetadataItem = TransformMetadataItem;


/***/ }),

/***/ "./src/transform-parameter.ts":
/*!************************************!*\
  !*** ./src/transform-parameter.ts ***!
  \************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.TransformParameter = void 0;
class TransformParameter {
    static transform(param) {
        var result = {
            'varname': param.key,
            'value': param.value
        };
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.TransformSensor = void 0;
/* eslint-disable @typescript-eslint/naming-convention */
const lodash = __webpack_require__(/*! lodash */ "./node_modules/lodash/lodash.js");
const transform_parameter_1 = __webpack_require__(/*! ./transform-parameter */ "./src/transform-parameter.ts");
const transform_metadata_item_1 = __webpack_require__(/*! ./transform-metadata-item */ "./src/transform-metadata-item.ts");
class TransformSensor {
    static transform(sensor) {
        var result = TransformSensor.doTransform(sensor);
        return result;
    }
    static transformContentSet(sensor) {
        var result = TransformSensor.doTransformContentSet(sensor);
        return result;
    }
    static doTransformContentSet(sensor) {
        var result = {};
        for (var p in sensor) {
    ...
    static mapProperty(name, sensor, result) {
    ...
    static convertWhitespace(input) {
    ...
    static soapValueTypeToResultType(type) {
        switch (type) {
            case 'BESDate':
                return 4;
            case 'DataSize':
    ...
    static _transformSensorQuery(query) {
        var scriptTypes = {
            'WMIQuery': 1,
            'BESRelevance': 2,
            'Powershell': 5,
    ...
    static platformStringToInt(platform) {
        switch (platform) {
            case 'Linux':
                return 1;
            case 'Mac':
...
exports.TransformSensor = TransformSensor;
TransformSensor._propertyMapContentSet = {
    'content_set': function (result, val) {
        result['content_set'] = {
            name: val.name
        };
    },
    ...
    'what_hash': 'what_hash',
    'category': 'category',
    'description': function (result, val) {
        if ((val || "") !== "") {
            result['description'] = TransformSensor.convertWhitespace(val);
        }
    },
    'hidden_flag': function (result, val) {
        if ((val || "") !== "") {
            if (val) {
    ...
    'delimiter': function (result, val) {
        if ((val || "") !== "") {
            result['delimiter'] = val;
        }
    },
    'parameters': function (result, val) {
        if ((val || "") !== "") {
            delete val.param;
            result['parameters'] = val;
        }
    },
    'queries': function (result, val) {
        // sort by os type
        val.sensor_query.sort((a, b) => (a.os > b.os) ? 1 : -1);
        // clean up query entry
        val.sensor_query.forEach((sensorQuery) => {
            var query = sensorQuery.query;
...
TransformSensor._propertyMap = {
    ...
    'ignore_case_flag': function (result, val) {
    ...
    'parameter_definition': function (result, val) {
        if ((val || "") !== "") {
            result['parameters'] = {
                parameter_text: val
            };
    ...
    'parameters': function (result, val) {
        if (val) {
            var params = lodash.map(val, (p) => {
                ...
                if (p && p.parameter) {
                    return transform_parameter_1.TransformParameter.transform(p.parameter);
                }
                else {
                    return null;
                }
    ...
    'subcolumns': function (result, val) {
        if ((val || "") === "") {
            result['columns'] = '';
        }
        else {
    ...
    'metadata': function (result, val) {
        if ((val || "") === "") {
            ...
            if (val.length === 1) {
                result['meta_data'] = {
                    meta_data_item: transform_metadata_item_1.TransformMetadataItem.transform(val[0]),
                };
            }
            else {
                result['meta_data'] = {
                ...
                for (var i = 0; val && i < val.length; i++) {
                    result['meta_data']['meta_data_item'].push(transform_metadata_item_1.TransformMetadataItem.transform(val[i]));
                }
            }
        }
    }
