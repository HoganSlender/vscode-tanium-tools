...
interface ServerServerGroupState {
...
export async function collectServerServerGroupInputs(config: WorkspaceConfiguration, context: ExtensionContext) {
    ...
    const state = {} as Partial<ServerServerGroupState>;
    await collectInputs('Compare Tanium Server Groups to Tanium Server Groups', state, steps);
    ...
    // store data
    return state as ServerServerGroupState;
}
