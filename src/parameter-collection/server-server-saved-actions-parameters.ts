...
interface ServerServerSavedActionState {
...
export async function collectServerServerSavedActionInputs(config: WorkspaceConfiguration, context: ExtensionContext) {
    ...
    const state = {} as Partial<ServerServerSavedActionState>;
    await collectInputs('Compare Tanium Server Saved Actions to Tanium Server Saved Actions', state, steps);
    ...
    // store data
    return state as ServerServerSavedActionState;
}
