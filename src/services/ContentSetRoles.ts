...
export class ContentSetRoles {
    static async analyzeContentSetRoles(left: vscode.Uri, right: vscode.Uri, context: vscode.ExtensionContext) {
        ...
        const missingContentSetRoles = await this.getMissingContentSetRoles(left.fsPath, right.fsPath);
        const modifiedContentSetRoles = await this.getModifiedContentSetRoles(left.fsPath, right.fsPath);
        const createdContentSetRoles = await this.getCreatedContentSetRoles(left.fsPath, right.fsPath);
        const unchangedContentSetRoles = await this.getUnchangedContentSetRoles(left.fsPath, right.fsPath);
        ...
        panelMissing.webview.html = this.getMissingWebContent(missingContentSetRoles, panelMissing, context, config);
        panelModified.webview.html = this.getModifiedWebContent(modifiedContentSetRoles, panelModified, context, config);
        panelCreated.webview.html = this.getCreatedWebContent(createdContentSetRoles, panelCreated, context, config);
        panelUnchanged.webview.html = this.getUnchangedWebContent(unchangedContentSetRoles, panelUnchanged, context, config);
    ...
    static getMissingContentSetRoles(leftDir: string, rightDir: string): Promise<any[]> {
    ...
    static getModifiedContentSetRoles(leftDir: string, rightDir: string): Promise<any[]> {
        const p: Promise<string[]> = new Promise((resolve, reject) => {
            const files: string[] = fs.readdirSync(leftDir);
            var modified: any[] = [];
    ...
    static getCreatedContentSetRoles(leftDir: string, rightDir: string): Promise<any[]> {
        const p: Promise<string[]> = new Promise((resolve, reject) => {
    ...
    static getUnchangedContentSetRoles(leftDir: string, rightDir: string): Promise<any[]> {
        const p: Promise<string[]> = new Promise((resolve, reject) => {
            const files: string[] = fs.readdirSync(leftDir);
            var unchanged: any[] = [];
    ...
    static getMissingWebContent(missingContentSetRoles: any[], panel: vscode.WebviewPanel, context: vscode.ExtensionContext, config: vscode.WorkspaceConfiguration): string {
        // get fqdns
        ...
        const html = compiledFunction({
            myTitle: 'Content Set Roles',
            panelWebviewCspSource: panel.webview.cspSource,
            nonce: nonce,
            missingItems: missingContentSetRoles,
            fqdns: fqdnsString,
    ...
    static getModifiedWebContent(modifiedContentSetRoles: any[], panel: vscode.WebviewPanel, context: vscode.ExtensionContext, config: vscode.WorkspaceConfiguration): string {
        // get fqdns
        const fqdnsString: string = config.get('fqdns', []).join();
        ...
        const html = compiledFunction({
            ...
            panelWebviewCspSource: panel.webview.cspSource,
            nonce: nonce,
            modifiedItems: modifiedContentSetRoles,
            fqdns: fqdnsString,
            usernames: usernamesString,
            signingKeys: signingKeysString,
            scriptUri: scriptUri,
    ...
    static getCreatedWebContent(createdContentSetRoles: any[], panel: vscode.WebviewPanel, context: vscode.ExtensionContext, config: vscode.WorkspaceConfiguration): string {
        // get fqdns
        const fqdnsString: string = config.get('fqdns', []).join();
        ...
        const html = compiledFunction({
            ...
            panelWebviewCspSource: panel.webview.cspSource,
            nonce: nonce,
            createdItems: createdContentSetRoles,
            fqdns: fqdnsString,
            usernames: usernamesString,
            signingKeys: signingKeysString,
            scriptUri: scriptUri,
    ...
    static getUnchangedWebContent(unchangedContentSetRoles: any[], panel: vscode.WebviewPanel, context: vscode.ExtensionContext, config: vscode.WorkspaceConfiguration): string {
        // Local path to main script run in the webview
        ...
        const html = compiledFunction({
            ...
            panelWebviewCspSource: panel.webview.cspSource,
            nonce: nonce,
            unchangedItems: unchangedContentSetRoles,
            scriptUri: scriptUri
        });
